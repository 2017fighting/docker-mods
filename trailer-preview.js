...
// @author       You
// @match        https://github.com/XingyiHua2024/Emby-Javascript-Details/blob/main/list_page_trailer.js
// @match        https://emby.raenzo.com/*
...
(function () {
    ...
    .has-video .cardOverlayContainer{
    ...
    document.addEventListener("viewbeforeshow", function (e) {
        ...
        if (e.detail.contextPath.startsWith("/list/") ||
            ...
            const selectorStr = e.detail.contextPath.startsWith("/videos?") ? `[data-index="1"].itemsTab .virtualItemsContainer` : ".virtualItemsContainer";
            ...
            const setupObserver = (itemsContainer) => {
                ...
                async function processAllChildren(container, token, refresh) {
                    ...
                    for (let node of children) {
                        ...
                        if (node.classList.contains('virtualScrollItem') && node.classList.contains('backdropCard')) {
    ...
    async function addTrailer(node, refresh = false) {
        ...
        if (refresh) {
            ['mouseenter', 'mouseleave', 'click'].forEach(type => {
        ...
        const playPreview = ()=>{
             if (trailerUrl.includes('youtube.com') || trailerUrl.includes('youtu.be')) {
                ...
                cardBox.classList.add('has-video');
        ...
        const clickPreview = ()=>{
            if (isDesktop()) return;
            if (cardBox.classList.contains('has-video')) return;
            event.stopImmediatePropagation(); // 阻止其它 click 处理器执行
            playPreview();
        }
        ...
        const mouseleaveHandler = () => {
            ...
            isHovered = false;
            imgContainer.classList.add('has-trailer');
            cardBox.classList.remove('has-video');
            img.style.filter = '';
        ...
        imgContainer.addEventListener('click', clickPreview);
        imgContainer._clickHandler = clickPreview;
    }
